...
def write_timeline(run_dir: Path) -> Path:
    ...
    lines.extend(
        [
            f"- Git commit: `{environment.get('git_commit') or '확인 불가'}`",
            f"- 시작 시 작업 트리 변경: `{environment.get('git_dirty')}`",
            f"- Ensemble 소스 해시: `{environment.get('ensemble_source_hash') or '미기록'}`",
        ]
    )
