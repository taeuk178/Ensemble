...
def write_readme(run_dir: Path) -> Path:
    ...
    lines = [
        ...
        f"| 일반 검토 | {(manifest.get('counters') or {}).get('iterative_reviews', len(layout.iter_reviews(run_dir)))} / {limits.get('iterative_reviews', limits.get('review_rounds', '?'))} |",
        f"| 승격 / 시퀀스 | {(manifest.get('counters') or {}).get('promotions', len(layout.iter_promoted(run_dir)))} / {manifest.get('last_review_round', 0)} |",
        f"| 최종 독립 검토 | {(manifest.get('counters') or {}).get('final_blind_attempts', len(layout.iter_blinds(run_dir)))} / {limits.get('final_blind_attempts', '?')} |",
