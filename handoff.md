...
- `manifest.json`: 상태, 현재 라운드, 요청 모델과 응답 모델, CLI 버전, 요청 텍스트 해시, 시작/종료 시각, 종료 사유, 사용량을 기록한다.
...
- 리뷰 N: `drafts/round-(N-1).md`를 읽고 `reviews/round-N.json`을 생성
- 수정 N: 리뷰 N을 반영해 `drafts/round-N.md`를 생성
- 리뷰 N에서 승인되면 `drafts/round-(N-1).md`가 블라인드 검증 대상이 되며 불필요한 새 draft를 만들지 않음
  ...
  "resolved_issues": [
    {
      ...
      "evidence_refs": ["drafts/round-1.md#인증-오류-처리"]
    }
  ],
  "questions_for_user": [],
  "nonblocking_risks": []
