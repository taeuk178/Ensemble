...
└── ensemble/
    └── runs/
        └── <run-id>/
            ...
            │   └── final-blind.json
            ├── review-sessions/
            │   └── <request-hash>/       # 일반 검토 세션이 이어서 읽는 최신 입력
            ├── panel/
            │   ├── <issue-id>/
            │   │   ├── gpt.json          # 독립 평가
            │   │   ├── agy.json          # 독립 평가
...
- `manifest.json`: 상태, 초안·검토 번호, 일반 검토 세션 ID와 요청 해시, 사용한 모델과 CLI 버전, 재시도, 시작·종료 시각, 종료 이유, 사용량을 기록한다.
- `review-sessions/<request-hash>/`: 같은 요청의 일반 검토가 이어서 사용하는 고정 작업 폴더다. 매 검토 전에 허용된 최신 입력만 덮어쓴다.
...
| 모듈 | 하는 일 | 단계 |
|---|---|---|
| `providers` | Codex·Agy 실행, 연결 확인, 시간 제한, 모델·버전 기록 | 1 (Agy는 3) |
...
일반 검토를 처음 실행할 때의 Codex 호출 기준 형태:

```text
codex exec
  --ignore-user-config
  --skip-git-repo-check
  -C <run>/review-sessions/<request-hash>
...
다음 일반 검토부터는 첫 호출의 `thread_id`를 사용해 `codex exec resume <session-id> -`로 이어간다. 세션 ID는 요청 해시와 실행 ID에 함께 묶어 기록하며, 둘 중 하나라도 다르면 재개를 거부한다.

- 리뷰 프롬프트는 stdin으로 전달하고 사용자 원문은 셸 인자에 포함하지 않는다.
- 검토 입력 폴더는 Git 저장소가 아니므로 이 내부 호출에만 `--skip-git-repo-check`를 사용한다.
...
- 일반 검토만 같은 요청 안에서 세션을 이어 쓴다. 제안, 추가 판단, 이슈 점검, 최종 독립 검토는 `--ephemeral` 새 세션으로 실행한다.
...
추가 판단이 필요할 때만 Antigravity CLI(`agy`)를 호출한다. 기준 형태는 다음과 같다.
...
Codex와 달리 Antigravity CLI에는 다음이 없으므로 래퍼가 보완한다.

- `--output-schema`가 없다. 스키마 본문을 프롬프트에 포함하고 응답을 `review.py`가 `jsonschema`로 로컬 검증한 뒤 실패 시 재시도한다.
- `--ignore-user-config`에 대응하는 단일 플래그가 없다. 평가 호출은 허용된 파일만 들어 있는 임시 입력 폴더에서 `--mode plan --sandbox`로 실행한다.
- `--ephemeral`이 없다. `--continue`나 `--conversation`을 사용하지 않고 매 평가를 새 비대화형 호출로 실행한다.
- 작업 디렉터리는 Codex와 동일한 격리 검토 입력 묶음을 사용한다.

**`--mode plan`만으로 격리가 끝난다고 보지 않는다.** `--sandbox`를 함께 사용하더라도 Codex의 `--sandbox read-only --ephemeral --ignore-user-config`와 동등하지 않다. 격리가 중요한 프로젝트는 네트워크·파일 접근을 제한한 별도 실행 환경을 사용한다.
...
| Antigravity CLI (`agy`) | 1.1.5 | 2026-07-22 |
...
**전체 이슈 기록을 리뷰어에게 파일로 전달하지 않는다.** 일반 검토 세션은 이어지므로 GPT가 자신이 앞서 낸 의견과 점수를 기억할 수는 있다. 다만 전체 원장, 내부 지표, 다른 평가자의 점수를 매 라운드 다시 주입하지 않아 불필요한 선입견이 더 커지는 것을 막는다.
...
전체 수치 이력과 다른 평가자의 점수는 **제3 평가의 통제된 재평가 단계에서만 의도적으로 공개**한다(§11.5). 같은 세션의 GPT는 자신의 이전 출력만 대화 이력으로 참고한다. 제3 평가에서 집계 카드를 보여주는 것은 상호 판단을 확인하려는 절차의 목적 자체다.

### 8.3 선입견 제거의 한계

이 조치로 수치 선입견이 완전히 없어지지는 않는다. 같은 세션의 GPT는 자신의 이전 판단을 기억하고, 열린 이슈의 위치도 알 수 있다. 이는 앞선 문답을 이어가고 이전 이슈를 조용히 누락하지 않게 하려는 의도된 절충이다. 전체 원장과 다른 평가자의 점수는 계속 숨긴다.
...
먼저 구조를 정확히 진술한다. 일반 검토의 외부 평가자는 **GPT 한 명뿐**이다. 작성자 Claude는 안정성 지표에서 제외되고(§10), Agy는 의견 대립 시에만 소집된다(§11.5). 따라서 평상시에는 불일치도 `d`를 계산할 패널 자체가 없다.
...
| `PANEL_DISSENT` | 이슈 | 추가 판단 후 GPT와 Agy의 판단이 갈림 |
...
5. 일반 검토는 같은 요청의 세션을 이어서 이전 문답을 참고한다. `APPROVED`가 나오면 새 ephemeral 세션으로 블라인드 최종 검증한다.
...
1. 해당 이슈에 한해 Agy의 Gemini 3.6 Flash를 독립 평가자로 투입한다.
2. 제3 평가자는 `request.md`, `rubric.md`, 현재 draft, 쟁점 섹션만 받는다. 기존 리뷰 JSON과 `decisions.md`는 받지 않는다.
3. 모든 독립 평가가 나온 뒤에만 집계 카드를 만들어 각 외부 평가자에게 배포하고 재평가를 1회 받는다. 재평가 결과는 평가자별 파일(`revotes/gpt.json`, `revotes/agy.json`)로 따로 저장해 감사와 재현이 가능하게 한다.
4. 재평가 결과로 판정한다.
...
Antigravity CLI(`agy`)가 없는 환경에서는 추가 판단을 수행할 수 없다. 이 경우 `BILATERAL_DEADLOCK` 상태로 곧장 §11.6의 사용자 결정 절차로 넘어가며, 제3 평가자 없이 임의 판정하지 않는다.
...
R2-I3  [추가 판단 재평가: 외부 평가자 2인]
  독립 평가 severity: GPT 4 / Agy 2   불일치도 2
...
- Agy: severity 2 — 초기 범위에서는 재로그인으로 충분
...
- Antigravity CLI(`agy`) 설치 및 로그인. 3단계 이후에만 필요하다. 추가 판단이 발생하지 않으면 호출되지 않으므로, 없으면 경고 후 2자 구도로 진행하고 의견 대립 시 §11.6의 사용자 결정 절차로 이관한다.
...
| 리뷰어에게 노출되는 점수 | 현재 입력 파일에는 없음. 같은 GPT 세션의 이전 출력은 기억할 수 있음 (추가 판단 재평가는 예외) |
| 일반 리뷰 라운드 상한 | 8 |
| 추가 판단 소집 상한 | 3 |
...
| 세션 보존 | 일반 검토만 같은 요청·실행 안에서 유지. 나머지는 `--ephemeral` |
...
- [ ] Agy 미설치 시 2자 구도로 경고 후 진행
- [ ] Agy 응답의 로컬 스키마 검증 실패 시 재시도
...
- [ ] 일반 검토 세션이 요청 해시와 실행 ID가 모두 같을 때만 재개됨
- [ ] 제안·추가 판단·이슈 점검·최종 독립 검토는 일반 검토 세션을 재사용하지 않음
...
- [ ] Agy 부재 시 임의 판정 없이 사용자 결정 절차로 이관
...
1. Agy preflight, 모델 고정, 격리 수준 실측
