...
> - [2층: 결과물 품질 비교](eval/handoff-layer2.md) — draft-0 대 final.md를 제3 모델이 블라인드 비교. 저비용.
...
- 논리 호출이 최종 실패하면(`InfraError`/`SchemaError` 전파) 그 호출이 소모한 토큰은 예외 경로에서 회수할 수 없다. `calls_unreported`로만 센다. 따라서 `calls_unreported > 0`이면 집계는 하한값이며, 1층 지표 출력에 그 사실을 표시한다.
...
3. **2단계 — 2층 `eval-quality`**: draft-0 대 final.md 블라인드 비교. 완료된 실행 1건으로 검증.
...
| 항목 | 선택지 | 영향 |
|---|---|---|
| 3층 예산 | full 세트 1회 = 파이프라인 8~12회 실행 비용 | 케이스 수와 실행 빈도. 현재 full 세트는 4케이스(스모크 3 + 품질 1)뿐이며, §2.4 권장 구성까지 채우려면 이 예산 결정이 필요하다 |
